...
const CSV_HEADER = 'id,title,completed,category,priority,createdAt,dueDate,notes,parentId';
...
export function exportToCSV(todos: Todo[]): string {
  const rows = todos.map(
    (t) =>
      `${escapeCSVField(t.id)},${escapeCSVField(t.title)},${t.completed},${t.category},${t.priority},${t.createdAt},${t.dueDate ?? ''},${escapeCSVField(t.notes ?? '')},${escapeCSVField(t.parentId ?? '')}`,
  );
...
export function importFromCSV(text: string): Todo[] {
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    const parentId = rest.length > 2 ? rest[2] : undefined;
